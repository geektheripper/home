...
planetarian::node::install-yarn() {
  NODE_VERSION=${1=$(planetarian::node::lstest-yarn)}
...
planetarian::node() {
  action=$1
  shift
  planetarian::node::$action $@
}

pltr_node=planetarian::node
